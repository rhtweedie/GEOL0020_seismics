...
tAB <-0.05409
tBA <- 0.06543
V1 <- 398 #calculated from direct wave - find_v1_ew.R

# read data
data_to_load <- "data/ew_data.csv"
...
ggplot(minus_data, aes(x = offset, y = tA_minus_tB)) +
  ...
  geom_smooth(data = minus_data[9:22,], aes(x = offset, y = tA_minus_tB), method = lm, se = FALSE) +
  xlab("Offset (m)") +
  ylab("tA - tB") +
...
model_2 <- lm(minus_data[9:22,]$tA_minus_tB ~ minus_data[9:22,]$offset)
...
ggplot(plus_minus_data, aes(x = offset, y = depth)) +
  ...
  geom_label(label = 'SURFACE', x = 37, y = 0.4) +
  ggtitle('Cross-Section of Depth of First Layer Boundary across Site of Suspected Barrow, EW') +
  xlab('Offset (m)') +
  ylab('Depth (m)') +
  annotate("rect", xmin = 25, xmax = 35, ymin = 0, ymax = 10, alpha = 0.2) +
  annotate("text", label = 'Surface width of barrow', x = 19, y = 2, size = 5) +
  annotate("segment", x = 23.75, xend = 28, y = 2, yend = 1.5)
